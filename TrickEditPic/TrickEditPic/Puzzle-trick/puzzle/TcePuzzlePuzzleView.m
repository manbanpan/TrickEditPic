...
- (void)TcePuzzleResetStyle
{
    if (_points)
    {
        ...
        for (int j = 0; j<subPoints.count; j++)
        {
            ...
            UIImage *image = self.TcePuzzleAssetArray[j];
                ...
                if (pointArray.count > 2) {
                    for(int i = 0; i < [pointArray count]; i++)
                    {
                        ...
                        if (pointString){ //&& gapString) {
                            ...
                            if (self.borderValue) {
                                if (point.x == 10) {
                                    point.x += self.borderValue * 2;
                                }
                                if (point.y == 10) {
                                    point.y += self.borderValue * 2;
                                }
                                if (point.x == 710) {
                                    point.x -= self.borderValue * 2;
                                }
                            ...
                            if (self.grpValue) {
                                point.x = point.x + (gap.x * self.grpValue * 2);
                                point.y = point.y + (gap.y * self.grpValue * 2);
                            }
